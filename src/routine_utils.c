...
/*   Updated: 2025/07/22 10:05:32 by nyousfi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void print_for_forks(t_philo *philo)
{
	...
	pthread_mutex_unlock(philo->dead_mutex);
	printf("%s%ld%s %d %s%s%s\n", PURPLE, get_current_time_ms()
		- philo->start_time, RESET, philo->id, YELLOW, "has taken a fork", RESET);
	printf("%s%ld%s %d %s%s%s\n", PURPLE, get_current_time_ms()
		- philo->start_time, RESET, philo->id, ORANGE, "has taken a fork", RESET);
	pthread_mutex_unlock(philo->print_mutex);
}
